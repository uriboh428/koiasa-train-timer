...
**最新バージョン: Ver 3.6 (完全ロール分離 ＆ 家族安心ゼロトラスト版)**

[![Python 3.10+](https://img.shields.io/badge/python-3.10+-blue.svg)](https://www.python.org/downloads/)
[![Streamlit](https://img.shields.io/badge/Streamlit-1.30+-FF4B4B.svg)](https://streamlit.io/)
[![Tests Passing](https://img.shields.io/badge/tests-30%2F30%20passing-brightgreen.svg)]()
[![Security](https://img.shields.io/badge/Security-Zero%20Trust%20Role%20Isolation-059669.svg)]()
...
9. **🔐 完全ロール分離 ＆ 家族安心ゼロトラスト設計 (Ver 3.6)**:
   * 誰が開いても（パスワードログイン後でも、ワンタップ共有URLでも）初期起動時は必ず安全な「一般モード」で起動。
   * 日常の設定欄（「⚙️ 出発タイミング ＆ 乗換設定」）からパスワード変更・共有URL再発行・Secrets設定などの管理操作を**完全撤去**。
   * 管理者メニューは画面最下部に独立して配置し、パスワード認証時のみオンデマンドで展開。ブラウザのキャッシュ無効化ヘッダーを配備。
...
* **Ver 3.6 (最新版)**: 完全ロール分離＆家族安心ゼロトラスト版。誰が開いても初期状態は一般モードで起動し、日常設定欄から危険な管理ボタンを完全撤去。管理者メニューは画面最下部に独立配置し、パスワード認証時のみオンデマンド展開。キャッシュ無効化タグを配備し確実な最新リリース配信を実現。
