...
**最新バージョン: Ver 4.1 (JR東日本公式 北朝霞ダイヤ完全同期 ＆ 祝日・土日・平日ダイヤ完全自動適応版)**

[![Python 3.10+](https://img.shields.io/badge/python-3.10+-blue.svg)](https://www.python.org/downloads/)
[![Streamlit](https://img.shields.io/badge/Streamlit-1.30+-FF4B4B.svg)](https://streamlit.io/)
[![Tests Passing](https://img.shields.io/badge/tests-44%2F44%20passing-brightgreen.svg)]()
[![Design](https://img.shields.io/badge/Design-European%20Sunrise%20%26%20Sunflower-EA580C.svg)]()
[![Security](https://img.shields.io/badge/Security-Dual%20Password%20Separation-059669.svg)]()

---

## 📖 プロジェクト概要

東京都国分寺市（**恋ヶ窪駅**）と埼玉県朝霞市（**北朝霞駅**）の間を、**西武国分寺線 ➡ JR中央線快速 ➡ JR武蔵野線**の3路線を乗り継いで移動する日常の通勤・通学を強力にサポートするWebアプリケーションです。
...
1. **⏱️ 特大リアルタイム・カウントダウン時計 (Ver 3.2)**:
   * 「あと〇分〇秒」を2.9remの特大文字で表示。歩行中や屋外の太陽光の下でも一目で残り時間が分かります。
...
9. **🔐 完全ロール分離 ＆ 家族安心ゼロトラスト設計 (Ver 3.6)**:
   * 誰が開いても初期起動時は必ず安全な「一般モード」で起動。日常の設定欄から危険な管理ボタンを完全撤去。
   * 管理者メニューは画面最下部に独立して配置し、パスワード認証時のみオンデマンドで展開。
10. **🛡️ ログイン・管理者パスワード完全分離（二重防壁アーキテクチャ）(Ver 3.7)**:
    * ご家族に共有する「一般ログインパスワード」と、設定変更・URL再発行を行う「管理者用マスターパスワード」を完全分離。
    * 画面最下部の管理者メニューは「管理者用マスターパスワード」でのみ解錠でき、一般パスワードでは厳格に遮断。
11. **🌻 朝日＆ひまわり・欧州サンシャインデザイン (Ver 3.8 - Aube Solaire & Tournesol)**:
    * コペンハーゲンや南仏プロヴァンスなど欧州最先端のウォームモダンUIトレンドを取り入れ、奥様がお好きなオレンジ色を軸に「朝日の光」と「ひまわり」をイメージした爽やかなデザインシステムへ全面刷新。
    * 朝の自然光を感じるサンライト・エクリュ背景（`#FDFBF7`）、夜明けのテラコッタ・グラデーションカード、ひまわりゴールドのアクセントを調和。
12. **⚡ 発車後自動次便更新＆スマートトランジション (Ver 3.9 - 駅発車標UX)**:
    * 駅の発車標（案内板）のように、カウントダウンが `00分00秒` に達して電車が発車した瞬間に「発車しました（次の便へ更新中...）」へバッジ表示が自動で変化。
    * 発車後約2秒で、画面全体が滑らかに再読み込みされ、次の電車のカウントダウンが自動スタート。
    * 「クライアント自律JavaScript」「Streamlit Fragment」「親スクリプト安全弁」の三重防護アーキテクチャにより、ユーザーが手動でリロードしなくても、画面を開いたままで常に最新の電車情報が表示され続けます。
...
flowchart TD
    ...
    V8["【Ver 3.5】管理者権限保護（ロール分離の概念導入）"]
    V9["【Ver 3.6】完全ロール分離＆家族安心ゼロトラスト版（全開起動廃止・設定欄からの完全撤去・最下部オンデマンド認証・キャッシュ対策）"]
    V10["【Ver 3.7】ログイン・管理者パスワード完全分離版（閲覧用PWと管理用マスターPWの二重防壁・個別変更・後方互換対応）"]
    V11["【Ver 3.8】欧州サンシャインデザイン版（朝日＆ひまわりの爽やか配色・Aube Solaireパレット・視認性と温もりの両立）"]
    V12["【Ver 3.9】発車後自動次便更新＆スマートトランジション版（駅発車標UX・00秒即時案内・三重防護自律リロード）"]
    V13["【Ver 4.0】最終便高コントラスト視認性向上＆ログイン画面完全プライベート秘匿版"]
    V14["【Ver 4.1】JR東日本公式 北朝霞ダイヤ完全同期＆祝日・土日・平日ダイヤ完全自動適応版"]
...
* **Ver 3.3**: 時間連動スマートデフォルト機能。アクセス時刻（JST）に応じて、1:00〜12:00は「恋ヶ窪 ➡ 北朝霞」（出勤便）、12:01〜24:59（翌0:59）は「北朝霞 ➡ 恋ヶ窪」（帰宅・深夜便）を自動選択。ワンアクションで即座に目的の電車情報を確認可能に。
* **Ver 3.4**: Goodpatch流UI/UX全面刷新。人間中心設計（HCD）に基づき、ブランド統合トップバー、すりガラス調ディープオーシャン・ヒーローカード、路線カラーメトロインフォグラフィック、洗練されたフォームUIを配備。
* **Ver 3.5**: 管理者権限保護版。家族利用と管理者操作の分離に着手。
* **Ver 3.6**: 完全ロール分離＆家族安心ゼロトラスト版。誰が開いても初期状態は一般モードで起動し、日常設定欄から危険な管理ボタンを完全撤去。管理者メニューは画面最下部に独立配置し、パスワード認証時のみオンデマンド展開。キャッシュ無効化タグを配備し確実な最新リリース配信を実現。
* **Ver 3.7**: ログイン・管理者パスワード完全分離版。ご家族に教える「一般ログインパスワード」と、設定管理を行う「管理者用マスターパスワード」の二重防壁アーキテクチャを導入。一般パスワードによる管理者メニューへの侵入を物理遮断し、管理者メニュー内で両パスワードを個別に変更可能に。旧形式設定からの後方互換性も100%担保。
* **Ver 3.8**: 欧州サンシャインデザイン版（Aube Solaire & Tournesol）。欧州の最先端ウォームモダン・ミニマリズムを取り入れ、奥様がお好きなオレンジ色を軸に「朝日」と「ひまわり」をモチーフとした爽やかなデザインシステムへ全面刷新。温かみのあるエクリュ背景、夜明けのテラコッタ・グラデーションカード、ひまわりゴールドのアクセントによって、毎朝の通勤・移動を心地よく照らすデザインを実現。
* **Ver 3.9**: 発車後自動次便更新＆スマートトランジション版。駅の発車標（案内板）のように、カウントダウンが `00分00秒` に達して電車が発車した瞬間に「発車しました（次の便へ更新中...）」へバッジ表示が自動で変化。発車後約2秒で、画面全体が滑らかに再読み込みされ、次の電車のカウントダウンが自動スタート。「クライアント自律JavaScript」「Streamlit Fragment」「親スクリプト安全弁」の三重防護アーキテクチャにより、ユーザーが手動でリロードしなくても、画面を開いたままで常に最新の電車情報が表示され続けます。
* **Ver 4.0**: 最終便高コントラスト視認性向上 ＆ ログイン画面完全プライベート秘匿版。終電案内に濃色すりガラスバッジ・ゴールド＆ホワイト文字を採用しWCAG AAA基準の圧倒的判読性を実現。さらに未認証・ログイン画面から駅名（恋ヶ窪・北朝霞）や運行情報を完全秘匿化し、個人の生活圏・通勤プライバシーを鉄壁に保護。
* **Ver 4.1 (最新版)**: JR東日本公式 北朝霞ダイヤ完全同期 ＆ 祝日・土日・平日ダイヤ完全自動適応版。内閣府祝日法に完全準拠したカレンダーエンジンと午前5時境界の鉄道運行日管理を配備。さらにJR東日本公式の最新北朝霞駅時刻表（平日・土休日）を完全同期し、東所沢止まりの除外や実効終電接続（平日23:30発、休日23:15発）まで1秒の狂いもなく案内。全44項目の包括テストを完全クリア。
