...
**最新バージョン: Ver 3.7 (ログイン・管理者パスワード完全分離版)**
...
10. **🛡️ ログイン・管理者パスワード完全分離（二重防壁アーキテクチャ）(Ver 3.7)**:
    ...
    * アプリのログイン画面では一般・管理者のどちらでも解錠可能。
    * 画面最下部の管理者メニューは「管理者用マスターパスワード」でのみ解錠でき、一般パスワードでは厳格に遮断。
    * 管理者メニュー内で一般ログインパスワードと管理者パスワードを個別に変更可能。
...
* **Ver 3.7 (最新版)**: ログイン・管理者パスワード完全分離版。ご家族に教える「一般ログインパスワード」と、設定管理を行う「管理者用マスターパスワード」の二重防壁アーキテクチャを導入。一般パスワードによる管理者メニューへの侵入を物理遮断し、管理者メニュー内で両パスワードを個別に変更可能に。旧形式設定からの後方互換性も100%担保。
