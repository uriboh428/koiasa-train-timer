...
* **Ver 3.3 (最新版)**: 時間連動スマートデフォルト機能 ＆ Goodpatch流UI/UX全面刷新。アクセス時刻（JST）に応じた出勤・帰宅便の自動初期選択（1:00〜12:00は恋ヶ窪発、12:01〜24:59は朝霞台発）に加え、ブランド統合トップバー、すりガラス調ディープオーシャン・ヒーローカード、路線カラーメトロインフォグラフィック、洗練されたフォームUIを配備。日常のスマホ利用における「使いたい」「洗練されている」体験を極限まで追求。
