...
flowchart TD
    ...
    V8["【Phase 8: 管理者権限保護＆家族安心】<br>Ver 3.5 (最新)<br>・一般/管理者ロール完全分離<br>・家族画面から管理操作を完全隠蔽<br>・オンデマンド管理者パスワード認証"]
...
| **Ver 3.5** | 管理者権限保護＆家族安心設計 | パスワード変更やSecrets設定を家族に触られたくない・誤操作を防ぎたい | セッションロール分離（`is_admin`）を導入。ご家族のワンタップURLセッションではパスワード変更・共有URL再発行・Secrets設定を完全隠蔽。設定欄下部のパスワード認証でのみオンデマンド昇格可能に | ご家族の誤操作リスクを100%排除し、管理機能の安全性を両立 |
